/*
   Low level functions that perform the final message encoding and interface with the SI5351 Transmitter
    using a 27MHz txco
   This code supports two different methods of interfacing with the Si5351.
    1. Using the Etherkit SI5351 library (etherkit defined)
       Signal is not as strong
       Can be used on any clock (but the signal is noisy on some clocks)
       Can be used with any TXCO
    2. Manually setting registers based on information by the ClockBuilder program for the Si5351
        (etherkit is commented out)
        This method can only be used on clock 2&3 (HF0 and HF1 on NIBBB 225)
        Only works with 27MHz txco
        Can only be set to frequencies defined in geofence.h - See APRSFreqs below
*/

#define VXCO_PIN 3


#define etherkit  // must stay undefined - this method is not debugged
...
#ifndef etherkit
#define VHF_XMIT_CLK0 SI5351_CLK2 // do not change
#define VHF_XMIT_CLK1 SI5351_CLK3  // do not change
  ...
  si5351b_revb_register_t const si5351b_revb_registers[num_reg] =
      {
          ...
          {0x0099, 0x00},
          {0x009A, 0x00},
          {0x009B, 0x00}, // vxco gain.  30 40 50
          {0x00A2, 0x69}, //             69 83 04
          {0x00A3, 0xEE}, //             EE BC 38
          {0x00A4, 0x01}, //             01 02 03
          {0x00A7, 0x00},
          {0x00A8, 0x00},
          {0x00B7, 0xD2}
...
void SetTestFrequency()
{ // Set frequency to 144.45MHz
  ...
  Serial.println(test_frequency);
  test_frequency = 144450000ULL*100ULL;
  override_flag = true;
}
...
#ifdef etherkit
  si5351.output_enable(VHF_XMIT_CLK0, 0); 
    si5351.output_enable(VHF_XMIT_CLK1, 0); 
#endif 
}
...
void VHF_on()
{
  ...
  else
  {
    si5351.set_freq(Frequency, VHF_XMIT_CLK0); 
      Serial.println(Frequency/100000000ULL);
  }
